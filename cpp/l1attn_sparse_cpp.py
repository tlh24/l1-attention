...
class L1AttnSparseFn(Function):
	...
	def forward(ctx, v, q, k, coo, dst_mxlen):
		...
		q = q.contiguous(); 
		k = k.contiguous();
		vo,attn = l1attnSparse_drv_cpp.forward(v, q, k, coo, dst_mxlen)
	...
	def backward(ctx, dvo):
		...
		d_v, d_q, d_k = l1attnSparse_drv_cpp.backward(dvo, v,q,k,coo,attn,dst_mxlen)
